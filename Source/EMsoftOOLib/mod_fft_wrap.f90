...
contains
  ...
  !--------------------------------------------------------------------------
  recursive function FFTWisdomType_Name(EMsoft,fileExists) result(name)
  ...
  implicit none
    ...
    wisdomFile = EMsoft%getConfigParameter('fftwWisdomfilename')
  ...
  !--------------------------------------------------------------------------
  recursive subroutine FFTWisdomType_Load(this, EMsoft)
  ...
  implicit none
    ...
    if(.not.this%loaded) then ! if we've already loaded the file we're done
      name = FFTWisdomType_Name(EMsoft, fileExists) ! get the wisdom file name and check if it exists
      if (fileExists) then ! make sure there is a file to read
        ...
        if (status .eq. 0) call Message%printError('FFTWisdomType_Load','error reading wisdom from file') ! handle error
      endif
  ...
  !> @date 08/06/19 WCL converted to object oriented
  !--------------------------------------------------------------------------
  recursive subroutine FFTWisdomType_Save(this, EMsoft)
