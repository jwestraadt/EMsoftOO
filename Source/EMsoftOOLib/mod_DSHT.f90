...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  implicit none
    ...
    ! set some constants
      ...
      call Message%printError('SH_cosLats','type must be "lambert" or "legendre" (got "' // trim(t) // '")')
    end if
  ...
  implicit none
    ...
    do j = -do, do ! loop over first dimension of square grid
      do i = -do, do ! loop over second dimension of square grid
        ...
        call Lambert%setxyzd( dc(:,i,j) )
        ierr = Lambert%LambertSphereToSquare( xy )
        xy = xy * do 
  ...
  implicit none
    ...
    if (ringID.gt.d) call Message%printError('SH_readRing','requested ring number does not exist')
  ...
  implicit none
    ...
    if (ringID.gt.d) call Message%printError('SH_writeRing','requested ring number does not exist')
  ...
  implicit none
    ...
    if (INFO.ne.0) then 
      if (INFO.gt.0) then
        io_int(1) = INFO
        call Message%WriteValue(' The following matrix argument had an illegal value',io_int,1)
      else
        io_int(1) = -INFO
        call Message%WriteValue(' The following U matrix element is zero :',io_int,1,'(I2,$')
        call Message%printMessage('; Matrix factorization completed, but solution could not be computed.')
      end if
      call Message%printError('SH_computeWeightsSkip','Error returned from LAPACK dgesv routine ... ')
    end if
    deallocate(IPIV)
    ...
    if (delta.gt.(epsilon(1.D0)**(1.D0/3.D0))/64.D0) then 
        write (*,*) 'delta = ',delta
        call Message%printError('SH_computeWeightsSkip','insufficient precision to accurately compute ring weights')
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  implicit none
    ...
    if (d.lt.1) call Message%printError('DiscreteSHTConstants_Init','square lambert half side length must be at least 1')
    if (trim(layout).eq.'legendre') then
      limit = 2*d
      if (l.ge.limit) call Message%printError('DiscreteSHTConstants_Init',&
                                              'maximum bandwidth is side # rings - 1 (for Legendre latitudes)')
    else if (trim(layout).eq.'lambert') then
      if (l.gt.this%d) call Message%printError('DiscreteSHTConstants_Init','maximum bandwidth is (Lambert square side length) / 2')
    else
      call Message%printError('DiscreteSHTConstants_Init','type must be "lambert" or "legendre" (got "' // trim(layout) // '")')
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  implicit none
    ...
    if (limL.ge.this%shtLut%maxL) then
      call Message%printError('SH_synthesize','limL must be < maxL')
    end if
    if (limL.eq.0) then 
